...
describe("Authentication", () => {
  ...
  it("should be not able to access privete routes when not authenticated", async () => {
    const response = await request(app).get("/dashboard");

    expect(response.status).toBe(401);
  });

  it("should be not able to access privete routes when provide a invalid token for authentication", async () => {
    const response = await request(app)
      .get("/dashboard")
  ...
  it("should receibe email notification when authenticated", async () => {
    ...
    console.log(transport.sendMail.mock.calls[0][0]);
